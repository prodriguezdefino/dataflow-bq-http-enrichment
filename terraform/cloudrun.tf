...
resource "null_resource" "build_and_push_cloud_run_image" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      gcloud builds submit --config "${path.module}/../cloud_run/cloudbuild.yaml" --substitutions=_IMAGE_NAME=${var.region}-docker.pkg.dev/${var.project_id}/${google_artifact_registry_repository.docker_repo.repository_id}/dataflow-http-app:latest "${path.module}/../cloud_run/app"
    EOT
  }
}
