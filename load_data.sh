...
INPUT_TABLE_FULL_ID=$(terraform -chdir=terraform output -raw input_table_id)

# Parse the table ID into the format expected by bq load.
INPUT_TABLE_ID=$(echo ${INPUT_TABLE_FULL_ID} | sed 's/projects\/\(.*\)\/datasets\/\(.*\)\/tables\/\(.*\)/\1:\2.\3/')
